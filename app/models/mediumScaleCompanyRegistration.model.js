...
Model.create = (newModel, result) => {
    ...
    sql.beginTransaction(function (err) {
        ...
        sql.query(usersDataQuery, usersData, function (err, rows) { //ok
            if (err) {
                ...
                sql.query(usersAccountsQuery, usersAccountsData, (err, rows) => { //ok
                    if (err) {
                        sql.rollback(function () {
                            throw err;
                        });
                ...
                sql.query(usersAddressQuery, usersAddressData, (err, rows) => { //ok
                    if (err) {
                        sql.rollback(function () {
                            throw err;
                        });
                ...
                sql.query(usersBusinessQuery, usersBusinessData, function (err, rows) { //ok
                    if (err) {
                        sql.rollback(function () {
                            throw err;
                        });
